...
export const updateCard = async (req, res, next) => {
  try {
    ...
    const updatedCard = await service.updateCardService(req.params.cardId, req.body, user._id)
    return res.status(200).json(successResponse("Cập nhật từ vựng thành công.", updatedCard));
  } catch (error) {
    next(error);
  }
};
...
export const deleteCard = async (req, res, next) => {
  try {
    ...
    return res.status(200).json(successResponse("Xóa từ vựng thành công.", null));
  } catch (error) {
    next(error);
  }
};
