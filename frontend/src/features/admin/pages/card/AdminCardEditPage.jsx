...
function AdminCardEditPage({ deckId, topicId, cardId, onNavigate }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true)
        const [deckRes, topicsRes, cardRes] = await Promise.all([
          getAdminDeckByIdApi(deckId),
          getDeckTopicsApi(deckId),
          getDeckCardByIdApi(deckId, cardId)
        ])
        ...
        setForm({
          ...
          phonetics: (card.phonetics || []).map((item) => ({
            text: item.text || '',
            locale: item.locale || 'en-US',
            audio: item.audio || '',
            fileName: ''
          })),
          explanationEn: card.explanation?.en || '',
    ...
    if (deckId && topicId && cardId) loadData()
  }, [deckId, topicId, cardId])
      ...
      <div className="admin-card-form-top">
        <div>
          <div className="admin-breadcrumbs">
            ...
            <span className="breadcrumb-separator">›</span>
            <span className="breadcrumb-active">{t('admin.cardEditTitle')}</span>
          </div>
          <h1 className="admin-card-form-title">{t('admin.cardEditTitle')}</h1>
        </div>
      ...
      <div className="admin-card-form-grid">
        ...
        <section className="admin-card-form-panel admin-card-detail-panel">
          ...
          <div className="admin-card-two-columns">
            <label className="admin-card-field"><span>{t('admin.cardExplanationEnLabel')}</span><textarea value={form.explanationEn} onChange={(event) => updateField('explanationEn', event.target.value)} rows={4} /></label>
            <label className="admin-card-field"><span>{t('admin.cardExplanationViLabel')}</span><textarea value={form.explanationVi} onChange={(event) => updateField('explanationVi', event.target.value)} rows={4} /></label>
          </div>
          ...
          <div className="admin-card-examples">
            ...
            <label className="admin-card-field"><span>{t('admin.cardExampleEnLabel')}</span><textarea rows={3} value={form.exampleEn} onChange={(event) => updateField('exampleEn', event.target.value)} /></label>
            <label className="admin-card-field"><span>{t('admin.cardExampleViLabel')}</span><textarea rows={3} value={form.exampleVi} onChange={(event) => updateField('exampleVi', event.target.value)} /></label>
          </div>
        </section>
      </div>
