...
export default function useDictation(lessonId) {
    ...
    const fetchData = async () => {
      try {
        ...
        const [segmentsResult, progressResult] = await Promise.all([
          ...
          axiosInstance.get(`/users/me/lessons/${lessonId}/segments-progress`).catch(() => ({ data: { data: [] } })),//test
          //axiosInstance.get(`/users/me/lessons/${lessonId}/segments-progress`),//real
        ]);
        const segments = segmentsResult.data.data;
        const progressList = progressResult.data.data || [];//test
        //const progressList = progressResult.data.data || [];//real

        const map = Object.fromEntries(progressList.map((p) => [p.segmentId, p]));
        ...
        const firstIncomplete = segments.findIndex((s) => !map[s._id]?.dictation?.completed);
  ...
  const submitSegment = async (segmentId, userInput) => {
